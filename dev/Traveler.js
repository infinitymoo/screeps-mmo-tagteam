...
class Traveler {
    ...
    static checkAvoid(roomName) {
        return Memory.Traveler.rooms[roomName] && Memory.Traveler.rooms[roomName].avoid && !_.includes(Memory.diplomacy.allowedRooms, roomName);
    }
...
Creep.prototype.MoveOffRoad = (function(target, opts = {}) {
    let priority = 1;
    let range = 3;
    if (!target) {
        return ERR_INVALID_ARGS;
    ...
    if (this.pos.getRangeTo(target) > opts.range) {
        return this.Move(target, {range:opts.range,priority:opts.priority});
    }
    Traveler.registerTarget(this, target, opts.range, opts.priority);
    return Traveler.moveOffRoad(this, target, opts.range, opts.priority);
});
